...
class Media
{
    ...
    public static function downloadById($file_id, $size_type = 'original', $not_found_img = '404.png', $inline_content = false, $quality = 90, $width = false, $height = False)
    {
        ...
        {
            // must be && , if one of them be false and another be 100 
            // then if $not_found_img_path exists
            // below code will create of that image another image 
            // with one coordinate of ZERO ( (int) false === 0  )
            if ($width || $height) 
            {
                if (file_exists($not_found_img_path))
                {
