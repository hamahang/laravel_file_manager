...
class Media
{
    static function get_file_content($not_found_default_img_path, $type = 'png', $text = '404', $bg = 'CC0099', $color = 'FFFFFF', $width = '640', $height = '480')
    {
        ...
        switch ($type)
        {
            ...
            case 'gif':
                ...
                return $img->encodeByExtension('gif');
                break;
            case 'jpg':
            case 'jpeg':
                $img = $manager->read($image);

                return $img->encodeByExtension('jpg');
                break;
        }
    }
            ...
            if ($crop_type != 'original')
            {
                ...
                switch ($crop)
                {
                    case "smart":
                        ...
                        break;
                    case "cover":
                        $res = $manager->read($file)->cover($OptionIMG['height'], $OptionIMG['width'])->save($tmp_path . '/' . $filename);
                        break;
                    case "resize":
                        $res = $manager->read($file)->resize($OptionIMG['height'], $OptionIMG['width'])->save($tmp_path . '/' . $filename);
                        break;
                }
                if (config('laravel_file_manager.Optimise_image'))
                {
    ...
    public static function downloadById($file_id, $size_type = 'original', $not_found_img = '404.png', $inline_content = false, $quality = 90, $width = false, $height = False)
    {
        ...
        if ($file)
        {
            ...
            $media_tmp_folder = config('laravel_file_manager.main_storage_folder_name') . '/media_tmp_folder/' ;
            $relative_tmp_path =  $media_tmp_folder. $file_name_hash;
            ...
            else
            {
                ...
                if (\Storage::disk($config)->has($file_path))
                {
                    ...
                    if (in_array($file_EXT, ['png', 'PNG', 'jpg', 'JPG', 'jpeg', 'JPEG']))
                    {
                        if ($width && $height)
                        {
                            $res = $manager->read($file_base_path)->cover((int)$width, (int)$height);
                            $res->save($tmp_path);
                            $res = $res->encodeByExtension($file_EXT, (int)$quality);
                        }
                        else
                        {
                            if ($quality < 100)
                            {
                                $res = $manager->read($file_base_path);
                                $res->save($tmp_path);
                                $res = $res->encodeByExtension('jpg', (int)$quality);
                            }
                            else
                            {
                                $res = $manager->read($file_base_path);
                ...
                else
                {
                    ...
                    if (file_exists($not_found_img_path))
                    {
                        ...
                        $relative_not_found_tmp_path = config('laravel_file_manager.main_storage_folder_name') . '/media_tmp_folder/' . $not_found_hash;
                        $relative_tmp_path =  $media_tmp_folder. $not_found_hash;
                        $tmp_path = $base_path . $relative_tmp_path;

                        if (!\Storage::disk(config('laravel_file_manager.driver_disk'))->has($relative_not_found_tmp_path))
                        {
                            $res = $manager->read($not_found_img_path)->cover((int)$width, (int)$height)->save($tmp_path);
                        }
                        if (!isset($res))
                        {
                        ...
                        //                        $res = $manager->read($not_found_img_path)->cover((int)$width, (int)$height)->encodeByExtension($ext, $quality);
        ...
        else
        {
            if ($width || $height)
            {
                if (file_exists($not_found_img_path))
                {
                    $not_found_ext = pathinfo($not_found_img_path, PATHINFO_EXTENSION);
                    $ext = ($not_found_ext != '') ? $not_found_ext : 'jpg';
                    $res = $manager->read($not_found_img_path)->cover((int)$width, (int)$height)
                        ->encodeByExtension($ext, $quality);
                }
                else
                {
                    $res = self::get_file_content($not_found_default_img_path, 'png', '404', 'cecece', 'FFFFFF', $width, $height);
            ...
            else
            {
                if (file_exists($not_found_img_path))
                {
                    $not_found_ext = pathinfo($not_found_img_path, PATHINFO_EXTENSION);
                    $ext = ($not_found_ext != '') ? $not_found_ext : 'jpg';
                    $res = $manager->read($not_found_img_path)->encodeByExtension($ext, $quality);
                }
                else
                {
